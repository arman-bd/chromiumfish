...
//! pipeline here is already close enough for the fingerprinter's
...
pub fn draw_stroke_text(pix: &mut Pixmap, opts: TextOptions<'_>) -> Result<()> {
    // For phase 1 we approximate stroke-text as fill-text (rare in the fingerprinter
...
/// that the fingerprinter and most pages use: "<weight>? <style>? <size>px <family>".
...
/// component only — for the canvas2d probes the fingerprinter runs (axis-aligned
...
fn blit_alpha_mask(
    ...
    // per-glyph blit transforms (good enough for the fingerprinter's
    ...
    // the only composite mode the fingerprinter's text probe uses. Other modes
