...
# ----- The the fingerprinter canvas probe op stream -----------------------------------
...
async def one_probe(send, recv, canvas_id_base):
    """Issue one full the fingerprinter canvas probe; returns (op_count, bytes_pulled)."""
...
async def main():
    ...
    if latencies_ms:
        ...
        print("=== per-probe latency (full the fingerprinter canvas probe roundtrip) ===")
