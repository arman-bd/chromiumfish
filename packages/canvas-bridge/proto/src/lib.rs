...
pub enum ClientMsg {
    ...
    /// overhead for fingerprinter-class probes which issue ~100 ops per canvas.
    Canvas2DBatch {
        id: CanvasId,
        ops: Vec<Canvas2DOp>,
    },
    ...
    /// so the browser-side WebGL extension hashes (the fingerprinter's webgl_extensions
...
/// Subset of canvas-2d API. We only need the ops the fingerprinter actually probes
...
/// Paint subset — the fingerprinter only ever uses CSS color strings, so we keep
...
/// WebGL op enum, minimal subset that covers the fingerprinter's WebGL fingerprint
